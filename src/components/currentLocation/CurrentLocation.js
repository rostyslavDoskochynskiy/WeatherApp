...
class CurrentLocation extends Component {
    ...
    componentDidMount() {
        ...
        if(currentLocation.country_name === 'Ukraine') return getLocationInfo('Kiev');
    ...
    renderWeatherForFiveDays = () => {
        ...
        return consolidated_weather.map((e, i) => {
            ...
            return <tr key={i} className="weather__table-tr">
                ...
                <td className="weather__table-td"><p className='high-temp'>{max_temp}<sup>&deg;</sup></p></td>
                <td className="weather__table-td"><p className='low-temp'>{min_temp}<sup>&deg;</sup></p></td>
            </tr>
        })
    };
    ...
    renderData = () => {
        ...
        const { country_code, country_name, city, latitude, longitude, state } = this.props.currentLocation;
        return locationChecking ? <div className="weather__spinner"><Spinner/></div> : <>
            <div className="current-location">
                ...
                <ul className="current-location__data">
                    <li className="current-location__item">Country CODE - <span className="current-location__data-item">{country_code}</span></li>
                    <li className="current-location__item">Country name - <span className="current-location__data-item">{country_name}</span></li>
                    <li className="current-location__item">City - <span className="current-location__data-item">{city}</span></li>
                    <li className="current-location__item">State - <span className="current-location__data-item">{state}</span></li>
                    <li className="current-location__item">Latitude - <span className="current-location__data-item">{latitude}</span></li>
                    <li className="current-location__item">Longitude - <span className="current-location__data-item">{longitude}</span></li>
                </ul>
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(CurrentLocation));