...
export const authCheck = () => async dispatch => {
    ...
    try {
        ...
        return dispatch (sessionUser ? authSuccess(JSON.parse(sessionUser)) : authFailed());
    } catch (e) {
...
export const signIn = userCredentials => async dispatch => {
    ...
    try {
        ...
        if(!userRegistered[login] === login && !userRegistered[password] === password) return signInFailed();
...
export const getCities = value => async dispatch => {
    try {
        ...
        const response = await axios.get(`${getTemporaryCors}https://www.metaweather.com/api/location/search/?query=${value}`);
        const {data} = response;
...
export const getLocationInfo = city => async dispatch => {
    ...
    try {
        ...
        const response = await axios.get(`${getTemporaryCors}https://www.metaweather.com/api/location/search/?query=${city}`);

        if (response.data.length > 0) {
            const dataCity = response.data.filter(e => e.title.toLowerCase() === city);
            const {woeid} = dataCity[0];
            const weather = await axios.get(`${getTemporaryCors}https://www.metaweather.com/api/location/${woeid}`);
            return dispatch({
