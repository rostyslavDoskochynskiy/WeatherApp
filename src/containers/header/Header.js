...
class Header extends Component {
    ...
    render() {
        ...
        const renderCities = allCities.map((e, i) => {
           return <li className="header__tips-items" onClick={e => handleChooseCity(e)} key={i}>{e.title}</li>
        });

        const { logOutUser, disabledAutoComplete, showCurrentLocation } = this.props;
        const items = [
            {
                name: 'Settings',
                action: ''
        ...
        const handleChooseCity = async e => {
            ...
            const { getLocationInfo, selectedLocation,  hideCurrentLocation } = this.props;
        ...
        return <div className="header" id="js-header">
            ...
            <div className="header__action-wrapper">
                <img className="header__action-icon" onClick={() => showCurrentLocation()} src="https://img.icons8.com/color/48/000000/map-pin.png"/>
                <DropdownComponent
                    trigger={['click']}
                    overlay={items}
                    container='js-header'
